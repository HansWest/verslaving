...
if($act) {
	...
	fclose($fp);
	header("Location: r_afspraken.php"); }
...
for ($i=0; $i<sizeof($list); $i++) {
	if ( $list[$i][0] >= time() ) {
		...
		if($list[$i][3] !== "birthday") { echo "<a href=\"r_afspraken.php?act=del&nr=".$list[$i][3]."\">delete</a>"; }
		echo "</td></tr>"; } }
?>
...
<form name="frm" method="post" action="r_afspraken.php?act=add">
<tr><td colspan="4">&nbsp;</td></tr>
<tr class="row"><td></td>
