...
if ($submit != "")
{
	...
	{
		$fout = "<br>Sorry! Maar als je <i>wel </i>wilt dat ik contact opneem met jouw arts en je geeft g&eacute;&eacute;n emailadres aan, dan geef je tegenstrijdige signalen <img src=\"../pics/macsmiley.gif\" alt=\"*glimlach*\">. Misschien wil je dat verbeteren?";
	}
	if ($partner =="1" && $pvoornaam == "" && $pachternaam == "")
	{
		echo "<br>jammer dat je deze site niet genoeg vertrouwt om jouw arts aan te geven <img src=\"../pics/macsmiley.gif\" alt=\"*glimlach*\">. Het was voldoende geweest om het kruisje niet(!) te zetten bij de vraag of je wilde dat jouw arts op de hoogte wordt gebracht...";
	}
	...
	if ($fout == "")
	{
		...
		if (!$result)
		{
			...
			$feedback = "<br>voornaam wordt: ". $pvoornaam."<BR>achternaam wordt: ".$pachternaam."<br>arts-<b>usernaam wordt: ". $pusernaam ;
			$feedback .= "</b><br>emailadres is: ". $pemailadres."<br>de geboortedatum is: ".$pgebdat."<br>de toon van praten is: ";
		...
		VALUES(NULL, '$zoekpersoon', '$pusernaam', '$pemailadres', '$pvoornaam', '$pachternaam', 'passwoord', '', '', 0, '$datetime', '$pgebdat','$pgender','$relatietoon', 0,'','','$opdehoogte','0')";
				$result = mysql_query($query);
...
///   Naar database wegschrijven
	...
	echo "<img src=\"../pics/macsmiley.gif\" border=\"0\"> <b>jouw arts, ".$voornaam." (als onderdeel van jouw team)</b><BR>";
	echo "Ik neem aan dat je een huisarts hebt, of een andere behandelend arts. Indien niet, dan raad ik je toch echt aan dat wel snel te regelen. Los van alloes is het gewoon verstandig om een huisarts te hebben.<br>";
	echo "Dat je hier de gegevens van jouw arts invult betekent niet (!) dat er automatisch contact met jouw arts wordt opgenomen! (en natuurlijk kan je deze gegevens veranderen in de toekomst)</p>";
	echo "<p>Ik vraag hiernaar omdat het soms handig is om medicatie aan te passen als mensen minder gaan drinken. Chemicali&euml;n kunnen elkaar versterken of verzwakken. Dus als jij jouw gebruik verandert dan kan het toch echt verstandig zijn om daar eens met iemand over te praten. Soms heeft gebruik ook emotionele gevolgen of heeft het een 'zelfmedicatie'-aspect <span class='rechtsbordertje'><img src='../pics/macsmiley.gif' border='0'> voor leesvoer over zelfmedicatie zie: <a href='http://www.helpdisk.nl/x.php?x=20470' target='_blank' class='menubutton'>www.HelpDisk.nl</a>/x?x=20470.</span>";
	echo "<br><i>er kunnen natuurlijk echt hele goede redenen zijn om niets te willen zeggen tegen jouw (huis)arts aan de andere kant is er een zwijgplicht... dus ik neem aan dat jij er over nadenkt voor je aanklikt of je dit project samen wilt werken met jouw -huis-arts of niet)</i></p>";
}else{
	include "layoutinc/inc.nltemplate1.php";
}
?>
...
if ($fout != "")
{
	...
	$vraag02 = "<tr><td>&nbsp;</td><td>&nbsp;</td><td><b>gegevens van jouw arts</b></td></tr>";
	$vraag02 .= "<tr><td><img src=\"../pics/mensje.gif\" alt=\"mens\"  width=\"10\" height=\"12\"  border=\"0\"></td><td>voornaam:</td><td><input name=\"pvoornaam\" type=\"text\" size=\"20\" maxlength=\"20\" value=\"".$net_voornaam."\"> (indien je die gebruikt)</td></tr>";
  ...
  if (isSet($net_gebdat))
  {
  	...
  	$vraag02 .= "<tr><td>&nbsp;</td><td valign=\"top\">geboortedatum:</td><td>dag:<input name=\"pgebdag\" type=\"text\" size=\"4\" maxlength=\"4\" value=\"".date('d', $gebdatum)."\"> maand: <input name=\"pgebmaand\" type=\"text\" size=\"4\" maxlength=\"4\" value=\"".date('m', $gebdatum)."\" > jaar: <input name=\"pgebjaar\" type=\"text\" size=\"4\" maxlength=\"4\" value=\"".date('Y', 	$gebdatum)."\"> (als je die toevallig zou weten)</td></tr>";
	}else{
	$vraag02 .= "<tr><td>&nbsp;</td><td valign=\"top\">geboortedatum:</td><td>dag:<input name=\"pgebdag\" type=\"text\" size=\"4\" maxlength=\"4\" value=\"dd\" onFocus=\"if(this.value=='dd')this.value='\".$gebdag.\"';\"> maand: <input name=\"pgebmaand\" type=\"text\" size=\"4\" maxlength=\"4\" value=\"mm\" onFocus=\"if(this.value=='mm')this.value='\".$gebmaand.\"';\"> jaar: <input name=\"pgebjaar\" type=\"text\" size=\"4\" maxlength=\"4\" value=\"jjjj\" onFocus=\"if(this.value=='jjjj')this.value='\".$gebjaar.\"';\"> (als je die toevallig zou weten)</td></tr>";
	}
///$vraag02 .= "<tr><td>&nbsp;</td><td valign=\"top\">geboortedatum:</td><td>dag:<input name=\"pgebdag\" type=\"text\" size=\"4\" maxlength=\"4\" value=\"dd\" onFocus=\"if(this.value=='dd')this.value='\".$gebdag.\"';\"> maand: <input name=\"pgebmaand\" type=\"text\" size=\"4\" maxlength=\"4\" value=\"mm\" onFocus=\"if(this.value=='mm')this.value='\".$gebmaand.\"';\"> jaar: <input name=\"pgebjaar\" type=\"text\" size=\"4\" maxlength=\"4\" value=\"jjjj\" onFocus=\"if(this.value=='jjjj')this.value='\".$gebjaar.\"';\"></td></tr>";
	$vraag02 .= "<tr><td>&nbsp;</td><td valign=\"top\">geslacht van jouw arts:</td><td><input type=\"radio\" name=\"pgender\" value=\"0\" ";
	if ($pgender == 0)
	{
		$vraag02 .= "checked";
	}
	$vraag02 .=	"> mijn arts is van het vrouwelijk geslacht<br><input type=\"radio\" name=\"pgender\" value=\"1\"";
	if ($pgender == 1)
	{
		$vraag02 .= "checked";
	}
	$vraag02 .= "> mijn arts is van het mannelijk geslacht</td></tr>";
	$vraag02 .= "<tr><td>&nbsp;</td><td>toon:</td><td>Wat is de toon waarin je met jouw arts spreekt?: <SELECT NAME=\"relatietoon\"><option value=\"2\">beste ...</option><option value=\"3\">geachte ...</option></td></tr>";
...
mijn arts mag op de hoogte worden gesteld van dit traject <font color=\"#800040\">*<br>&nbsp; &nbsp; &nbsp; (N.B. privacy wetgeving)</font></td></tr>";
...
als je samenwerking wenst met jouw arts dan is het aanvinken hier verplicht in verband met de wet op de privacy en persoonsgegevens. anders mogen wij geen berichten uitzenden naar derden.</i></font></td></tr>";
	...
	if ($submit != "")
	{
		...
		$sendtext = $temptext. "\n";
		$temptext = "Momenteel heb je als (huis)arts: $pvoornaam $pachternaam - een ";
		$sendtext .= $temptext;		
		if ($pgender == "")
		{
			echo "...(uhhh... is jouw behandelend arts <i>echt </i>geslachtsloos?)";
		}
		if ($pgender==1)
		{
			$temptext .= "man";
		...
		if ($pgebjaar < (date('Y') - 100) ) {
			echo "... (waardoor ik jouw behandelend arts wel een beetje uhhhh 'op leeftijd' vind... heb je wel <i>vier</i> cijfers ingevuld in het geboortejaar?)";
		}
		...
		if ($opdehoogte==1)
		{
			...
			$temptext .= " NIET op de hoogte wilt stellen van dit traject. Dus dat je (wat jouw -huis-arts betreft) deze poging liever alleen onderneemt. ";
			if ($pemailadres !="")
			{
